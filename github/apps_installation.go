...
// Both legacy and stateless installation tokens use the ghs_ prefix. The remainder
// is intentionally not validated because GitHub may change the stateless token
...
func ValidateInstallationToken(token string) error {
	...
	if len(token) <= len("ghs_") || token[:len("ghs_")] != "ghs_" {
		return fmt.Errorf("invalid installation token format: token must start with 'ghs_'")
	}
	return nil
}
